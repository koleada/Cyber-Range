...
The envionrment consists of two firewall routers connected via a point-to-point (/30) transit network, with each managing distinct trust zones. 
...
I chose this architecture to best mimic a modern enterprise network whereby there are deliberate trust bounaries/security zones separated by internal routed firewalls. 

### Security Zones and Trust Boundaries 

1. pfSense
   - Attacker machine is largely free to communicate with any machines in the vulnerable subnet
...
2. OPNSense
   ...
   - We allow external access into the network through a OpenVPN server hosted on our firewall.
   - We have a dedicated monitoring subnet where we host a Wazuh instance that watches over the AD enviornment.
      - All AD machines have the Wazuh agent installed on them which proivdes EDR.
      - I chose to isoalte the monitoring machine into its own network so we can craft very specific firewall rules to control the flow of traffic to this network. Unauthorized access to a machine controlling our EDR/SIEM would be bad news, so I wanted some practice secureing these critical systems.
...
The trust zones are as follows: 
   - VPN Subnet - conditional trust, users must have a valid configuration file with a certificate from our CA, they must also have a valid credential on our firewall. With that said, users may be connecting via personal devices which could inadvertantly bring security risks onto the network. We want to be careful of exactly what traffic is flowing from this subnet into the AD subnet, this subnet really should never communicate with the monitoring subnet (unless we want to eventually put EDR on VPN client devices).
   - AD Subnet - internal, trusted, but also highly sensitive. Again, this subnet would hold the majority of critical infrastrcuture and services in a real environment. We want to heavily monitor all devices and netwok traffic on this network, limit east-west traffic, minimize/heavily restrict inbound traffic, have very strcit policys regarding passwords, elevated privledges, protocol usage, etc, we also want to heavily collect all authentication and security related logs. 
   - Monitoring - internal, potentially the most trusted subnet, but also potentially the most highly resitriced. We want very specific firewall rules involving this submet, we really only want to allow EDR communication from machines on the AD network (takes place on one specific port for wazuh), HTTP connections again on a specific port to access the Wazuh web gui, SSH to allow remote management of the monitoring VM only from the minimal number of static IPs. 
    
3. The Transit Network
   - This is the boundary from our untrusted, DMZ network and the true internal network in which holds the keys to the kingdom.
   ...
   - The transit network is where we want to convert untrusted traffic into trusted traffic. We do this through very strict firewall rules on the OPNSense firewall/router depdning on the source, destination, and protocol.
   ...
   - This subnet should be minimal, only two addresses here, should not host any services, because it's traffic is untrusted.   
...
Due to virtualization constraints (switching is done automatically at the hypervisor or host network level) and modern enterprise best practices, security enforcement in this lab is performed primarily at Layer 3. Subnets are routed through firewall appliances rather than relying solely Layer 2 segmentation.
